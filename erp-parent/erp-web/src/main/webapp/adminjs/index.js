		...
		$.ajax({
			...
			success : function(rtn){
				if(rtn.success){
					...
					$.messager.alert('提示消息',rtn.message,'info');
				}
			}
		});
	});
...
function serverLogin() {
    ...
    $.ajax({
    	url :'empAction_updatePwd?oldPwd='+ $oldpass.val() +'&newPwd=' + $newpass.val(),
    	type:'post',
    	dataType : 'json',
    	success:function(rtn){
    		$.messager.alert('我的消息',rtn.message,'info');
        	if(rtn.success){
        		$oldpass.val('');
        		$newpass.val('');
        		$rePass.val('');
